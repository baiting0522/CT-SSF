...
class AbsErrorErrFunc(RegressionErrFunc):
    ...
    def apply(self, prediction, y, weights=[]):
...
class BaseModelNc(BaseScorer):
    def __init__(self, model, err_func, normalizer=None, beta=1e-6, weights=None):
    ...
    def score(self, x, y=None):
        ...
        if self.weights is not None:
            if len(self.weights) != n_test:
                raise ValueError("weights length must match number of test samples")
            norm *= self.weights
...
class FeatRegressorNc(BaseModelNc):
    def __init__(self, model,
                 ...
                 normalizer=None, beta=1e-6, g_out_process=None,weights=[]):
    ...
    def score(self, x, y):  # overwrite BaseModelNc.score()
        ...
        #ret_val = ret_val * attention_weights_list
        #ret_val = ret_val.numpy() / norm
        return ret_val
    ...
    def score_batch(self, dataloader):
        ...
        for x, _, y in tqdm(dataloader):
            ...
            #batch_ret_val = self.weights[0:len(batch_ret_val)] * batch_ret_val
    ...
    '''def predict(self, x, nc, significance=None):
        ...
        # Assuming score function exists and returns errors and attention weights
        _, attention_weights_list = self.score(x)
        ...
        if significance:
            ...
            if prediction.ndim > 1:
                ...
                lirpa_model = BoundedModule(self.model.model.g, torch.empty_like(z))
                # Use attention weights to modulate the perturbation
                attention_modulated_eps = attention_weights_list * feat_err_dist[0][0]
                ptb = PerturbationLpNorm(norm=self.feat_norm, eps=attention_modulated_eps)
                ...
                raise NotImplementedError("Unidimensional prediction not implemented.")

            return intervals

        else:
            raise NotImplementedError("Significance level must be specified.")'''


class TimeSeriesTransformerBlock(nn.Module):
    def __init__(self, embed_size, heads, dropout, forward_expansion):
        super(TimeSeriesTransformerBlock, self).__init__()
...
class TimeSeriesTransformer(nn.Module):
    def __init__(self, embed_size, heads, depth, seq_length, forward_expansion, dropout):
        ...
        for _ in range(depth):
            self.layers.append(
                TimeSeriesTransformerBlock(embed_size, heads, dropout, forward_expansion)
            )
...
class BaseIcp(BaseEstimator):
    ...
    '''def calibrate(self, x, y, increment=False):
        ...
        # Calculate scores and attention weights
        scores, attention_weights = self.nc_function.score(self.cal_x, self.cal_y)
        ...
        if self.conditional:
            ...
            for cond in self.categories:
                idx = category_map == cond
                # Use scores and attention weights to calculate weighted_scores
                cal_scores = scores[idx]
                weighted_scores = cal_scores * attention_weights[idx]  # Apply attention weights
                self.cal_scores[cond] = np.sort(weighted_scores, 0)[::-1]
        else:
            self.categories = np.array([0])
            weighted_scores = scores * attention_weights  # Apply attention weights
            self.cal_scores = {0: np.sort(weighted_scores, 0)[::-1]}
...
class IcpRegressor(BaseIcp):
    ...
    '''def predict(self, x, significance=None):
        ...
        prediction = np.zeros((x.shape[0], self.nc_function.model.model.out_shape, 2, n_significance) if n_significance > 1 else (x.shape[0], self.nc_function.model.model.out_shape, 2))
        _, attention_weights = self.nc_function.score(x) 

        condition_map = np.array([self.condition((x[i, :], None)) for i in range(x.shape[0])])
        for condition in self.categories:
            idx = condition_map == condition
            if np.sum(idx) > 0:
                weighted_scores = self.nc_function.predict(x[idx, :], self.cal_scores[condition], significance) * attention_weights[idx]
                prediction[idx, :] = weighted_scores  # Adjust predictions based on weighted scores
